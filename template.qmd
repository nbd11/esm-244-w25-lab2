...
execute: #execute functions control code chunks
  warning: false
  message: false
---
...
AIC<-AIC(mdl1,mdl2,mdl3)

AIC_table <-as_data_frame(AIC)
```
...
Break data into 10 randomized chunks, then train the data on 9 of those chunks for each model, testing it on the other 1 testing set. Repeat this process 10 times, then analyze which model has the highest accuracy across all three.
...
summarize our RMSE (which model was best on avg)
...
```{r}
#seed
# no such thing as a random # in comp sci
...
rmse_predict_test<-predict_test |> 
  summarize(rmse_md1=calc_rmse(mass,model1),rmse_md2=calc_rmse(mass,model2),rmse_md3=calc_rmse(mass,model3))
```
...
for( i in 1:folds){
  ...
  # separate into test and train
  kfold_test_df<-penguins_fold |> filter(group==i)
  kfold_train_df<-penguins_fold|> filter(group!=i)
...
kfold_cv<-function(i,df,formula){
  ...
  kfold_test_df<-df |> filter(group==i)
  kfold_train_df<-df |> filter(group!=i)
  
  # Run for one model
  kfold_training_lm1<-lm(formula,data=kfold_train_df)
  
  #Get the predictions from the model
  
  kfold_predict_test<-kfold_test_df |> 
  mutate(md = predict(kfold_training_lm1,kfold_test_df))
...
summary_total_model<-summary(total_model)
```
